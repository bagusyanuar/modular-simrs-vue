    ...
    modules.forEach((mod, i) => {
      ...
      if (tenantCode !== 'base') {
        ...
        if (existsSync(featureExtraPath)) {
          const extraVarName = `extra_mod_${i}`;
      ...
      if (featureEntries.length > 0) {
        entries.push(`  "${mod.name}": [${featureEntries.join(', ')}]`);
      }
    });
    
    // Check for Global Tenant Extra Routes (Highest Priority for custom pages)
    if (tenantCode !== 'base') {
      const extraPath = path.join(presentationBase, '_tenants', tenantCode, 'extra.routes.ts').replace(/\\/g, '/');
      if (existsSync(extraPath)) {
        const varName = `extra_routes`;
        imports.push(`import * as ${varName} from '${extraPath}';`);
        entries.push(`  "extra": [${varName}]`);
      }
    }
