...
import { createSSOClient } from '../api/client';
/**
 ...
 */
export function createSSOGuard(router, options) {
    ...
    router.beforeEach(async (to, _from, next) => {
        ...
        if (SSOSessionManager.hasPersistedSession() || SSOSessionManager.config.persistentStorage === 'cookie') {
            ...
            try {
                console.log('[SSOGuard] Attempting automatic refresh (Passive/Active)...');
                ...
                const tokens = await client.refreshToken(currentSession?.refreshToken ? { refresh_token: currentSession.refreshToken } : undefined);
                const newSession = {
                    accessToken: tokens.access_token,
                    refreshToken: tokens.refresh_token,
                };
                ...
                if (options.onAuthenticated)
                    options.onAuthenticated(newSession);
                return next();
            }
            catch (error) {
                console.warn('[SSOGuard] Automatic refresh failed. Proceeding to Silent Login flow.');
